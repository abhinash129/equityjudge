						...
						<div class="box-body">
							<table id="example2" class="table table-bordered table-hover">
								<thead>
									...
									<tbody>
									    <tr ng-repeat="roll in latestReportData | orderBy:sortType:sortReverse | filter:searchCompany">
									        <td>{{ roll.Sr_No }}</td>
									        <td>
									            <a href="${pageContext.request.contextPath}/graphpattern/graph/selectCompny?companyName={{ roll.COMPANYNAME }}">
									                <font color="black">{{ roll.COMPANYNAME }}</font>
									            </a>
									        </td>
									        <td>{{ roll.PATTERN_NAME }}</td>
									        <td>{{ roll.SUGGESTION }}</td>
									      <!--   <td>{{ roll.FORMING_DATE }}</td> -->
									        <td>{{ roll.CLOSE_PRICE }}</td> <!-- Add CURRENT_PRICE or CLOSE_PRICE here -->
									    </tr>
									</tbody>
