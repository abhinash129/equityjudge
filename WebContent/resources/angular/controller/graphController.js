...
App
		.controller(
				...
				[
						...
						'$filter',
						'WatchListService',
						function($scope, GraphService, $filter,
								WatchListService) {
							$scope.load1 = false;
							var id;
							...
							$scope.insertUserReview = function(review) {
								GraphService.insertUserReview(review).then(
										function(response) {
							...
							$scope.selectedAverage = 'average';  // This ensures the radio button is selected by default
							...
							$scope.checkDate = function(formingDate) {
								...
								if (forming_Date.getDate() == today_Date
										.getDate()) {
									return false;
								} else {
									return true;
							...
							$scope.addStockPortfolio = function addStockPortfolio() {
								...
								var pfid = $scope.pf_id;
								$scope.addStockFail = "";
								$scope.addStockSuccess = "";
								$scope.stockDataFromTrend = [];
								$scope.stockDataFromTrend.push(companyCode,
										buyDate, buyPrice, totalShares, pfid)
								var stocksFromTrendToPorfolio = $scope.stockDataFromTrend;
								GraphService
										.addToPortfolio(
												stocksFromTrendToPorfolio)
										.then(
												function(response) {
													var i = response;
													if (i == 0) {
													...
													function(response) {
														...
														if ($scope.suggestionList.length == 0) {
															...
															for (var i = 0; i < $scope.suggestionList.length; i++) {
																...
																$scope.suggestions
																		.push(companyListSuggestions.companyName
																				+ "("
																				+ companyListSuggestions.companyId
																				+ ") {"
																				+ companyListSuggestions.companyCode
																				+ "}");
																...
																.autocomplete(
																		{
																					...
																					ui) 
																			{
																				 ...
																				 * comments
																				 */
																				var FinalCompanyName = $scope.companyName;

																				/*window.location = path
																						+ '/graphpattern/graph/selectCompny?companyName='
																						+ FinalCompanyName;*/
							...
							$scope.passCompanyName = function(firstCompany,
									companyCode, companyId) {
								$scope.average = [];
								$scope.companyNameList = [];

								if (firstCompany != null && firstCompany != -2
										&& firstCompany != -3) {
									// console.log("1")
									$scope.sc_id = "(" + companyId + ") {"
											+ companyCode + "}";
								...
								$scope.companyNameList.push(
										CompanyNameFromSearchBox,
										$scope.sc_code);
								...
								GraphService
										.getValue(compnyList, year, average)
										.then(
												function(response) {
													...
													if (!($scope.companyName == 'BSE SENSEX')) {
														$scope
																.getSuggestion(CompanyNameFromSearchBox);
													}
													/* Data Generator */
													function DataGenarator() {
														...
														for (var int = 0; int < $scope.companyDetailsLists.length; int++) {
															...
															if ($scope.average[int] == '15') {
																avarageList
																		.push(movingAverage15);
															}
															if ($scope.average[int] == '30') {
																avarageList
																		.push(movingAverage30);
															}
															if ($scope.average[int] == '100') {
																avarageList
																		.push(movingAverage100);
															}
															if ($scope.average[int] == '200') {
																avarageList
																		.push(movingAverage200);
															}
															$scope.flag = false;
														}
														var list = [];
														...
														for (var int2 = 0; int2 < avarageList.length; int2++) {
															...
															list
																	.push(movingAvarageMap);
														}
														return list;
													}
													;

													$scope.labelName = angular
															.uppercase($scope.name);
													$scope.companyName = [];
													$scope.load = false;
													if (flagforEmpty == 0) {
														$scope.options = {
							...
							$scope.calculation = function(cmpnynames) {
								...
								GraphService
										.getQuotesValue(cmpny, '1 Year', avrg)
										.then(
												function(response) {
													$scope.data = response;
							...
							$scope.getSuggestion = function(
								...
								GraphService.getPatternSuggestion(
										suggestedCompanyName).then(
										function(response) {
							...
							$scope.passCompanyNameMain = function(firstCompany,
									companyCode, companyId) {
								$scope.average = [];
								$scope.companyNameList = [];
								$scope.companyName = firstCompany;
								...
								var average = $scope.average;
								$scope.companyNameList.push(
										CompanyNameFromSearchBox, companyCode);
								var compnyList = $scope.companyNameList;
								console.log(compnyList + "," + year + "-"
										+ average)
								GraphService
										.getValue(compnyList, year, average)
										.then(
												...
												function(response) {
													 ...
													 * var ClosedPrice = []; for
													 * (var int = 0; int <
													 * $scope.companyDetailsLists.length;
													 * int++) {
													 * $scope.companyDetailsList =
													 * $scope.companyDetailsLists[int];
													 * for( var i = 0; i <
													 * $scope.companyDetailsList.length;
													 * i++) { var
													 * companyWholeDetails =
													 * $scope.companyDetailsList[i];
													 * ClosedPrice.push({x: new
													 * Date(companyWholeDetails.stockDate),
													 * y:
													 * companyWholeDetails.companyClosePrice
													 * }); }} var list = [];
													 * //Line chart data should
													 * be sent as an array of
													 * series objects. var
													 * closePrice= { values:
													 * ClosedPrice, //values -
					...
					if (controller != null) {
						updateModel = function(value, picker) {
							return $scope.$apply(function() {
								return controller.$setViewValue(element
										.datepicker("getDate"));
							});
						};
						...
						controller.$render = function() {
							originalRender();
							return element.datepicker("setDate",
